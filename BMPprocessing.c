...
uint8_t *readImage(const char *fileIn, uint32_t *width, uint32_t *height, uint16_t ***brightness, uint32_t *unpaddedRowSize, uint16_t *bitsPerPixel)
{
    ...
    *brightness = (uint16_t **)malloc(sizeof(uint16_t*) * (*height)); // 2D array to store brightness
    for (uint32_t i = 0; i < *height; i++)
        (*brightness)[i] = (uint16_t*)malloc(sizeof(uint16_t) * (*unpaddedRowSize));
    uint16_t p = 1, k = 0, averageBrightness = 0;
...
uint8_t *sortPixels(uint32_t width, uint32_t height, uint8_t *pixels, uint16_t **brightness, uint32_t unpaddedRowSize)
{
    uint8_t *startPixels = pixels;
